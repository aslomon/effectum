...
describe("extractTags", () => {
  test("returns stack tags for nextjs-supabase", () => {
    const tags = extractTags({ appType: "other", stack: "nextjs-supabase", description: "" });
  ...
  test("returns stack tags for python-fastapi", () => {
    const tags = extractTags({ appType: "other", stack: "python-fastapi", description: "" });
  ...
  test("returns appType tags for web-app", () => {
    const tags = extractTags({ appType: "web-app", stack: "generic", description: "" });
  ...
  test("description keyword 'crm' adds crm and related tags", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "A CRM tool for managing customers" });
  ...
  test("description keyword 'dashboard' adds dashboard tags", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "Analytics dashboard for tracking sales" });
    assert.ok(tags.includes("dashboard"), "should include dashboard");
    assert.ok(tags.includes("data-visualization"), "should include data-visualization");
  });

  test("description keyword 'supabase' adds supabase and db-needed", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "A supabase backend service" });
    assert.ok(tags.includes("supabase"), "should include supabase");
    assert.ok(tags.includes("db-needed"), "should include db-needed");
  });

  test("description keyword 'stripe' adds payments and auth-needed", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "Integrate stripe payments" });
    assert.ok(tags.includes("payments"), "should include payments");
    assert.ok(tags.includes("auth-needed"), "should include auth-needed");
  });

  test("returns empty array for completely empty inputs", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "" });
  ...
  test("returns deduplicated tags (no duplicates)", () => {
    ...
    const tags = extractTags({ appType: "other", stack: "nextjs-supabase", description: "A Next.js nextjs app" });
    const unique = new Set(tags);
    assert.equal(tags.length, unique.size, "tags should not contain duplicates");
  });

  test("description is case-insensitive", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "CRM DASHBOARD Auth" });
    assert.ok(tags.includes("crm"), "should match 'CRM' case-insensitively");
    assert.ok(tags.includes("dashboard"), "should match 'DASHBOARD' case-insensitively");
    assert.ok(tags.includes("auth-needed"), "should match 'Auth' case-insensitively");
  });
});
...
describe("recommend() for Next.js + Web App", () => {
  ...
  test("includes stack-specific subagents for nextjs-supabase", () => {
    assert.ok(result.subagents.includes("frontend-developer"), "should include frontend-developer");
    assert.ok(result.subagents.includes("backend-developer"), "should include backend-developer");
    assert.ok(result.subagents.includes("postgres-pro"), "should include postgres-pro");
  });

  test("includes skills for frontend-heavy stack", () => {
    assert.ok(result.skills.includes("frontend-design"), "should include frontend-design skill");
    assert.ok(result.skills.includes("webapp-testing"), "should include webapp-testing skill");
  });
});
...
describe("recommend() for Python + API", () => {
  ...
  test("python stack includes backend-specific subagents", () => {
    assert.ok(pythonResult.subagents.includes("backend-developer"), "should include backend-developer");
    assert.ok(pythonResult.subagents.includes("api-designer"), "should include api-designer");
    assert.ok(pythonResult.subagents.includes("debugger"), "should include debugger");
  });

  test("python result does not include nextjs-specific subagents", () => {
    assert.ok(!pythonResult.subagents.includes("react-specialist"), "should not include react-specialist");
    assert.ok(!pythonResult.subagents.includes("nextjs-developer"), "should not include nextjs-developer");
  });

  test("python tags include python and api-first", () => {
    assert.ok(pythonResult.tags.includes("python"), "tags should include python");
    assert.ok(pythonResult.tags.includes("api-first"), "tags should include api-first");
  });

  test("python tags do not include nextjs", () => {
    assert.ok(!pythonResult.tags.includes("nextjs"), "tags should not include nextjs");
  });
});
...
describe("Foundation Commands are always in recommendations", () => {
  const alwaysCommands = COMMAND_RULES.filter((r) => r.always).map((r) => r.key);
  ...
  for (const stack of stacks) {
    for (const appType of appTypes) {
      test(`stack=${stack} appType=${appType} always includes foundation commands`, () => {
        const result = recommend({ stack, appType, description: "", autonomyLevel: "standard", language: "english" });
        for (const cmd of alwaysCommands) {
          assert.ok(result.commands.includes(cmd), `should include always-command: ${cmd}`);
        }
      });
    }
  }
...
describe("Foundation MCPs are always in recommendations", () => {
  ...
  test("always MCPs present for minimal input", () => {
    const result = recommend({ stack: "generic", appType: "other", description: "", autonomyLevel: "standard", language: "english" });
  ...
  test("always MCPs present for nextjs-supabase web-app", () => {
    const result = recommend({ stack: "nextjs-supabase", appType: "web-app", description: "dashboard", autonomyLevel: "standard", language: "english" });
    for (const mcp of alwaysMcps) {
      assert.ok(result.mcps.includes(mcp), `should always include MCP: ${mcp}`);
    }
  });
  ...
  test("always MCPs present for python-fastapi api-backend", () => {
    const result = recommend({ stack: "python-fastapi", appType: "api-backend", description: "REST API", autonomyLevel: "standard", language: "english" });
    for (const mcp of alwaysMcps) {
      assert.ok(result.mcps.includes(mcp), `should always include MCP: ${mcp}`);
    }
  });
...
describe("Description keywords influence tags", () => {
  test("'CRM' keyword adds crm tag", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "Build a CRM system" });
    assert.ok(tags.includes("crm"), "CRM keyword should add crm tag");
  });

  test("'saas' keyword adds multi-tenant, multi-user, payments tags", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "A SaaS product for teams" });
  ...
  test("'realtime' keyword adds realtime and websocket tags", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "A realtime chat" });
    assert.ok(tags.includes("realtime"), "realtime keyword should add realtime");
    assert.ok(tags.includes("websocket"), "realtime keyword should add websocket");
  });

  test("'agent' keyword adds ai-agent tag", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "AI agent pipeline" });
    assert.ok(tags.includes("ai-agent"), "agent keyword should add ai-agent");
  });

  test("'stripe' keyword adds payments and auth-needed", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "Integrate stripe for billing" });
  ...
  test("'mcp' keyword adds mcp and ai-agent tags", () => {
    const tags = extractTags({ appType: "other", stack: "generic", description: "Build an MCP server" });
...
describe("Autonomy levels — recommend() returns consistent shape", () => {
  ...
  for (const level of levels) {
    test(`autonomyLevel=${level} returns valid recommendation shape`, () => {
      ...
      assert.ok(Array.isArray(result.commands), `commands should be array for level=${level}`);
      assert.ok(Array.isArray(result.hooks), `hooks should be array for level=${level}`);
      assert.ok(Array.isArray(result.skills), `skills should be array for level=${level}`);
      assert.ok(Array.isArray(result.mcps), `mcps should be array for level=${level}`);
      assert.ok(Array.isArray(result.subagents), `subagents should be array for level=${level}`);
      assert.ok(Array.isArray(result.tags), `tags should be array for level=${level}`);
      assert.equal(typeof result.agentTeams, "boolean", `agentTeams should be boolean for level=${level}`);
    });
    ...
    test(`autonomyLevel=${level} foundation commands are still present`, () => {
      ...
      for (const cmd of alwaysCommands) {
        assert.ok(result.commands.includes(cmd), `level=${level} should include always-command: ${cmd}`);
      }
    });
  }
});
