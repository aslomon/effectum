# PRD: DESIGN.md Generation & `/design` Command
...
The MVP should introduce a dedicated `/design` command that:
...
### AC-1: `/design` Command
- New command file: `.claude/commands/design.md`
- Command asks for:
...
- Effectum recommends `/design` when `appType` is one of:
  - `web-app`
  - `mobile-app`
...
`/design` inspects existing files when available:
- `tailwind.config.*`
...
- `/plan` and frontend-related prompts should explicitly read `DESIGN.md` if present

### AC-6: Non-UI Safety
- No `DESIGN.md` generation for backend-only installs unless user explicitly runs `/design`
- Docs clarify that `DESIGN.md` is optional outside UI work

### AC-7: Documentation
- Add `docs/design-md.md` explaining:
  - what DESIGN.md is
  - when to use it
  - sample workflow
...
1. Should `/design` be generated by default after `/prd:new` for web projects, or only suggested?
