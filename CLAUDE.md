...
- GOAL-FIRST: define `GOAL.md` with outcome, mandate, workflow mode, evidence, and stop conditions before choosing plan/run/orchestrate (use `effect:goal`)
- SPEC-DRIVEN: define requirements and acceptance criteria before coding
- TEST-FIRST: write failing tests, then implement (use `effect:dev:tdd`)
- DONE = compiles + tests pass + linter clean (use `effect:dev:verify`)
- Plan complex features before implementing (use `effect:dev:plan`)
- Review changes before committing (use `effect:dev:review`)
- Fix build errors incrementally (use `effect:dev:fix`)
- Clean dead code periodically (use `effect:dev:refactor`)
...
| ---------------------- | -------------- | ---------------------------------------- |
| `effect:goal`          | Start          | Goal envelope + workflow-mode decision    |
...
| `effect:dev:e2e`       | QA             | E2E tests (Playwright / XCTest UI)       |
...
| `effect:dev:run`       | Full Auto      | Iterative autonomous implementation      |
| `effect:dev:diagnose`  | Diagnosis      | Post-mortem analysis of loop failures    |
| `effectum:init`        | Setup          | Populate project context in CLAUDE.md    |
| `effectum:explore`     | Setup          | 4 parallel agents → 7 knowledge docs    |
