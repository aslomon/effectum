...
| `status`       | enum   | Yes      | One of: `drafting`, `ready`, `in-progress`, `done`, `implemented`, `archived`. Updated by workflow commands.                   |
| `onboarded`    | bool   | No       | `true` if this PRD was auto-generated by `/onboard` from an existing codebase. Omit for manually written PRDs.                 |
...
> **Onboarded PRDs**: PRDs generated by `/onboard` use `status: implemented` (not `done`) and have `onboarded: true` in their frontmatter. All acceptance criteria are pre-checked `[x]` because they describe existing, working features. These PRDs serve as **protection rules** — new development must not break the behavior they document. After onboarding, use `/prd:new` for new features and `/prd:update` to modify existing ones.
